...
describe("Header Component Coverage Tests", () => {
  ...
  it("should hide search input on blur when empty", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByPlaceholderText("Buscar filmes...")
      ).not.toBeInTheDocument();
    });
  });

  it('should show favorites count badge when there are favorites', () => {
    // Mock favorites in context by providing them via localStorage
    ...
    // There might be only one link depending on the viewport
    expect(favoritesLinks.length).toBeGreaterThan(0);

    // Check that the count badge is displayed
  ...
  it("should initialize search query from URL params", () => {
    ...
    const mobileSearchInput = screen.getByPlaceholderText(
      "Buscar..."
    ) as HTMLInputElement;
